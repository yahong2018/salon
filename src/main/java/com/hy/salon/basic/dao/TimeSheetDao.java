...
public class TimeSheetDao extends BaseDAOWithEntity<TimeSheet> {
    ...
    //查询当月员工出勤记录
    public List<TimeSheet> getTSheetsByType(Long recordId, String time,String type) {
/*
...
*/
       int  timeSheetType = 0;
        if("chidao".equals(type)){
            timeSheetType = 1;
        }else if("zaotui".equals(type)){
            timeSheetType = 2;
        }
        String timeStart=time+"-01";
        String timeEnd = time +"-31";
        String where = "stuff_id=#{stuffId} and day between  #{timeStart} and  #{timeEnd} and time_sheet_type= #{timeSheetType}";
        Map map=new HashMap();
        map.put("where",where);
        Map<String, Object> parameters = new HashMap<>();
        parameters.put("stuffId", recordId);
        ...
        parameters.put("timeSheetType",timeSheetType);
