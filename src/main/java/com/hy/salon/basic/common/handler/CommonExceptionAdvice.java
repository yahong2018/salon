...
public class CommonExceptionAdvice {
  ...
  public AjaxResult handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException e){
    logger.error("传入参数格式不匹配异常", e);
    return new AjaxResult().failure(e.getName() + " value is " + e.getValue() + " required type is " + e.getRequiredType());
  }
  ...
  public AjaxResult handleServiceException(ServiceException e) {
    ...
    return new AjaxResult().failure(msg[0], com.alibaba.fastjson.JSONObject.parseObject(msg[1]));
  }
...
/*  @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
  ...
  public AjaxResult handleException(JSONException e) {
    logger.error("请求参数解析错误:", e);
    return new AjaxResult().failure(e.getMessage());
  }*/
